...
CUST = ['C1']  # customers
MFG_PROD = [(mfg, prod) for mfg in MFG for prod in PROD]
DISTR_PROD = [(distr, prod) for distr in DISTR for prod in PROD]
# Product demand
...
# stock variables
D_MFG_Stk = pulp.LpVariable.dicts("MFG_stk", ((i, j) for i, j in MFG_PROD), lowBound=0, upBound=None,
                                  cat='Integer')
D_DISTR_Stk = pulp.LpVariable.dicts("DISTR_stk", ((i, j) for i, j in DISTR_PROD), lowBound=0, upBound=None,
                                    cat='Integer')
# flow variables
D_MFG_DISTR_flow = pulp.LpVariable.dicts("MFG_DISTR_flow", ((i, j, k) for i, j, k in MFG_DISTR), lowBound=0,
                                         upBound=None,
                                         cat='Integer')
D_DISTR_CUST_flow = pulp.LpVariable.dicts("DISTR_CUST_flow", ((i, j, k) for i, j, k in DISTR_CUST), lowBound=0,
                                          upBound=None, cat='Integer')
D_MFG_CUST_flow = pulp.LpVariable.dicts("MFG_CUST_flow", ((i, j, k) for i, j, k in MFG_CUST), lowBound=0, upBound=None,
                                        cat='Integer')
# Objective function
...
var_cost = pulp.lpSum([MFG_VarCost[mfg, prod] * D_MFG_Stk[mfg, prod] for mfg, prod in MFG_PROD]) + \
           pulp.lpSum([DISTR_VarCost[distr, prod] * D_DISTR_Stk[distr, prod] for distr, prod in DISTR_PROD])

transport_cost = pulp.lpSum(MFG_DISTR[mfg, distr, prod] * D_MFG_DISTR_flow[mfg, distr, prod]
                            for mfg, distr, prod in MFG_DISTR) \
                 + pulp.lpSum(MFG_CUST[mfg, cust, prod] * D_MFG_CUST_flow[mfg, cust, prod]
                              for mfg, cust, prod in MFG_CUST) \
                 + pulp.lpSum(DISTR_CUST[distr, cust, prod] * D_DISTR_CUST_flow[distr, cust, prod]
                              for distr, cust, prod in DISTR_CUST)
